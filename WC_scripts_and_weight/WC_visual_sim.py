...
class TaskThread(QtCore.QThread):
	...
	random_state = numpy.random.RandomState(seed)
        ...
        if useTVBConnectome == True:
            ...
            TVB_elec        = np.array(tvb_elec)
            TVB_abs_syna    = np.array(tvb_abs_syna)
            TVB_signed_syna = np.array(tvb_signed_syna) 

            # now, save the TVB electrical and synaptic activities to separate files
            np.save("./output/tvb_neuronal.npy", TVB_elec)
            np.save("./output/tvb_abs_syn.npy", TVB_abs_syna)
            np.save("./output/tvb_signed_syn.npy", TVB_signed_syna)
