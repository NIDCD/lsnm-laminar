...
class TaskThread(QtCore.QThread):
    ...
    def run(self):
        ...
        neural_network = './output/neuralnet.json'
	...
	random_state = numpy.random.RandomState(seed)
        ...
        if model == "" and weights_list == "":
		    ...
		    # Here is where I am going to put in the Wang-Knosche Weight Modifications
   		    # connection weight boosts
		    #############################################################
	    	    # boosts LGN -> V1 connection weights BU E
	    	    lgnW = 2.0
	    	    ############################################################
    	            # boosts V1  -> V4 connection weights BU E (supragranular)  
	   	    v1w_s = 3.5 
	            # boosts V1  -> V4 connection weights BU E (infragranular)
		    v1w_d = 0.0 
	            #############################################################
	            # boosts V4  -> IT connection weights BU E (supragranular)
	            v4w_s = 3.625 
	            # boosts V4  -> IT connection weights BU E (infragranular)
		    v4w_d = 0.0 
		    #############################################################
		    # boosts IT  -> FS connection weights BU E (supragranular)
		    itw_s = 3.0
		    # boosts IT  -> FS connection weights BU E (infragranular)
		    itw_d = 0.0 # 
		    #############################################################
		    # boosts FS  -> D2 connection weights BU E
		    cued2w = 4.0 
		    #############################################################
		    # boosts FS  -> FR connection weights BU E (Deep/Infragranular) (1.25)
		    fsfr_d = 0.0
		    # boosts FS  -> FR connection weights BU E (Superficial/Supragranular) (1.25)
		    fsfr_s = 2.75
		    #############################################################
		    # boosts FS  -> D1 connection weights L (Superficial/Supragranular) I
		    cued1w_s = 1.75
		    # boosts FS  -> D1 connection weights L (Deep/Infragranular) I
		    cued1w_d = 1.75
		    #############################################################
		    # boosts D1  -> FS connection weights L  I
		    d1cuew = 1.5
		    #############################################################
		    # boosts D1  -> FR connection weights BU E (Superficial/Supragranular)
		    d1respw_s = 2.5
		    # boosts D1  -> FR connection weights BU E (Deep/Infragranular)
		    d1respw_d = 0.0 
		    #############################################################
		    # this is the one I am tinkering with
		    # boosts FR  -> D1 connection weights TD I (Deep/Infragranular) to supraganular D1 (2.25)
		    frd1_s = 2.0
		    # boosts FR  -> D1 connection weights TD I (Deep/Infragranular) to infraganular D1 (2.25)
		    frd1_d = 2.5
		    # this is next one I am going to tinker with with
		    # boosts FR  -> D2 connection weights TD I (Deep/Infragranular)  to supraganular D2 (2.0)
		    frd2_s = 5.0 
		    # boosts FR  -> D2 connection weights TD I (Deep/Infragranular) to infraganular D2 (2.0)
		    frd2_d = 4.0 
		    # D1 -> IT is the one I am mucking about with
		    #############################################################
		    #2 boosts D1  -> IT superficial connection weights TD I
		    d1_it_s = 0.5 
		    #2 boosts D1  -> IT deep connection weights TD I
		    d1_it_d = 3.0 
		    #############################################################
		    # boosts D2  -> IT superficial connection weights TD E
		    d2_it_s = 1.4
		    # boosts D2  -> IT deep connection weights TD E
		    d2_it_d = 0.4
		    #############################################################
		    # boosts D2  -> V4 connection weights TD E
		    d2_v4 = 1.5
		    #############################################################
		    # boosts IT  -> V4 connection weights TD E
		    it_v4 = 1.25
		    ############################################################# 
		    # boosts V4  -> V1 connection weights TD E superficial 
		    v4_v1_s = 1.5
		    #0 # boosts V4  -> V1 connection weights TD E infragranular
		    v4_v1_d = 2.5
		    #############################################################
		    # set the following two values to zero to restore a D2 higher than D1 connection weights
		    # boosts D2  -> D1 connection weights L  E this controls pyramidal to pyramidal (TD)
		    d2d1wpp = 1.4 
		    # boosts D2  -> D1 connection weights L  E this controls pyramidal to layer 4
		    d2d1wp4 = 0.0 
		    #############################################################
		    # boosts D1  -> D2 connection weights L  E this controls pyramidal to layer 4   (TD)
		    d1d2wp4 = 1.0 
		    # boosts D1  -> D2 connection weights L  E this controls pyramidal to pyramidal
		    d1d2wpp = 0.0 
		    ################################################################
		    # attention twiddle
		    attsspd2_mod = 1.0
		    attsdpd2_mod = 1.0
			    ...
			    if (origin_module == 'lgns') and (destination_module == 'ev1h' or destination_module == 'ev1v'):						
				destination[1] = destination[1]*lgnW 
				# modify connection weights from pyramidal cells to the excitatory units in V4				
				# bottom up connections			
			    elif (origin_module == 'spv1h' or origin_module == 'spv1v') and (destination_module == 'ev4h' or destination_module == 'ev4c' or destination_module == 'ev4v'):			
				destination[1] = destination[1]*v1w_s 
				# modify connection weights from supragranular pyramidal cells to the excitatory units in V4         		
				# bottom up connections
			    elif (origin_module == 'dpv1h' or origin_module == 'dpv1v') and (destination_module == 'ev4h' or destination_module == 'ev4c' or destination_module == 'ev4v'):			
				destination[1] = destination[1]*v1w_d 
				# modify connection weights from infragranular pyramidal cells to the excitatory units in V4         		
				# bottom up connections
			    elif (origin_module == 'spv4v' or origin_module == 'spv4h' or origin_module == 'spv4c') and (destination_module == 'exss'):			
				destination[1] = destination[1]*v4w_s 
				# modify connection weights between supragranular pyramidal V4 and IT
				# bottom up connections			
			    elif (origin_module == 'dpv4v' or origin_module == 'dpv4h' or origin_module == 'dpv4c') and (destination_module == 'exss'):			
				destination[1] = destination[1]*v4w_d 
				# modify connection weights between infragranular pyramidal V4 and IT
				# bottom up connections			
			    elif (origin_module == 'spss') and (destination_module == 'exfs'):
				destination[1] = destination[1]*itw_s 
				# modify connection weights between IT and 'Cue' region
				# bottom up connections
			    elif (origin_module == 'dpss') and (destination_module == 'exfs'):
				destination[1] = destination[1]*itw_d 
				# modify connection weights between IT and 'Cue' region
				# bottom up connections
			    elif (origin_module == 'dpfs' and destination_module == 'exfr'):
				destination[1] = destination[1]*fsfr_d 
				# modify connection weights between 'Cue' region and response units
				# bottom up connections
			    elif (origin_module == 'spfs' and destination_module == 'exfr'):
				destination[1] = destination[1]*fsfr_s 
				# modify connection weights between 'Cue' region and response units
				# bottom up connections
			    elif (origin_module == 'spfs' or origin_module == 'dpfs') and (destination_module == 'efd2'):
				destination[1] = destination[1]*cued2w 
				# modify connection weights between 'Cue' region and D2 units
				# bottom up connections
			    elif (origin_module == 'spd2' or origin_module == 'dpd2') and (destination_module == 'spd1' or destination_module == 'dpd1'):
				destination[1] = destination[1]*d2d1wpp 
				# modify connection weights between D2 region and D1 units			
				# top down connections				
			    elif (origin_module == 'spd1' or origin_module == 'dpd1') and (destination_module == 'efd2'):
				destination[1] = destination[1]*d1d2wp4
				# modify connection weights between D1 region and D2 units	
				# bottom up connections
			    elif (origin_module == 'spd1') and (destination_module == 'exfr'):
				destination[1] = destination[1]*d1respw_s
				# modify connection weights between supragranular D1 region and FR units	
				# bottom up connections
			    elif (origin_module == 'dpd1') and (destination_module == 'exfr'):
				destination[1] = destination[1]*d1respw_d
				# modify connection weights between infragranular D1 region and FR units	
				# bottom up connections
			    elif (origin_module == 'dpfs' and destination_module == 'did1'):
				destination[1] = destination[1]*cued1w_d 
				# modify lateral connection weights between Cue region and D1 units			
				# lateral connections
			    elif (origin_module == 'spfs' and destination_module == 'sid1'):
				destination[1] = destination[1]*cued1w_s 
				# modify lateral connection weights between Cue region and D1 units			
				# lateral connections
			    elif (origin_module == 'dpd1' or origin_module == 'spd1') and (destination_module == 'difs' or destination_module == 'sifs'):	
				destination[1] = destination[1]*d1cuew 
				# modify lateral connection weights between D1 region and Cue units			
				# lateral connections
			    elif (origin_module == 'spd1' or origin_module == 'dpd1') and (destination_module == 'exfr'):
				destination[1] = destination[1]*d1respw 
				# modify connection weights between D1 region and response units
				# bottom up connections
			    elif (origin_module == 'dpfr' and destination_module == 'did1'):	
				destination[1] = destination[1]*frd1_d 
				# modify top down connection weights between Response region and D1 units			
				# top down connections orginating in infragranular layer
			    elif (origin_module == 'dpfr' and destination_module == 'sid1'):	
				destination[1] = destination[1]*frd1_s 
				# modify top down connection weights between Response region and D1 units			
				# top down connections orginating in supragranular layer
			    elif (origin_module == 'dpfr' and destination_module == 'sid2'):	
				destination[1] = destination[1]*frd2_s 
				# modify to down connection weights between Response region and D2 units
				# lateral connections	
			    elif (origin_module == 'dpfr' and destination_module == 'did2'):	
				destination[1] = destination[1]*frd2_d 
				# modify top down connection weights between Response region and D2 units
				# lateral connections	
			    elif (origin_module == 'dpd1' and destination_module == 'dinss'):	
				destination[1] = destination[1]*d1_it_d 	
				# modify top down connections between D1 and IT
			    elif (origin_module == 'dpd1' and destination_module == 'sinss'):	
				destination[1] = destination[1]*d1_it_s 	
				# modify top down connections between D1 and IT
			    elif (origin_module == 'dpd2' and destination_module == 'spss'):	
				destination[1] = destination[1]*d2_it_s 	
				# modify top down connections between D2 and IT
			    elif (origin_module == 'dpd2' and destination_module == 'dpss'):	
				destination[1] = destination[1]*d2_it_d 	
				# modify top down connections between D2 and IT
			    elif (origin_module == 'dpd2' or origin_module == 'spd2') and (destination_module == 'spv4v' or destination_module == 'dpv4v' or destination_module == 'spv4h' or destination_module == 'dpv4h' or destination_module == 'spv4c' or destination_module == 'dpv4c'):
				destination[1] = destination[1]*d2_v4 	
				# modify top down connections between D2 and v4
			    elif (origin_module == 'spss' or origin_module == 'dpss') and (destination_module == 'spv4v' or destination_module == 'dpv4v' or destination_module == 'spv4h' or destination_module == 'dpv4h' or destination_module == 'spv4c' or destination_module == 'dpv4c'):
				destination[1] = destination[1]*it_v4 	
				# modify top down connections between IT and v4
			    elif (origin_module == 'dpv4v' or origin_module == 'dpv4h') and (destination_module == 'spv1v' or destination_module == 'spv1h'):
				destination[1] = destination[1]*v4_v1_s
				# modify top down connections between V4 and V1
			    elif (origin_module == 'dpv4v' or origin_module == 'dpv4h') and (destination_module == 'dpv1v' or destination_module == 'dpv1h'):
				destination[1] = destination[1]*v4_v1_d
				# modify top down connections between V4 and V1
			    elif (origin_module == 'spd2' or origin_module == 'dpd2') and (destination_module == 'efd1'):
				destination[1] = destination[1]*d2d1wp4
				# modify lateral connections between D2 pyramidal and D1 excitatory
			    elif (origin_module == 'spd1' or origin_module == 'dpd1') and (destination_module == 'spd2' or destination_module == 'dpd2'):
				destination[1] = destination[1]*d1d2wpp
				# modify lateral connections between D1 pyramidal and D2 pyramidal
			    elif (origin_module == 'atts' and destination_module == 'spd2'):
				destination[1] = destination[1]*attsspd2_mod
				# modify lateral connections between Attention and D2 superior pyramidal
			    elif (origin_module == 'atts' and destination_module == 'dpd2'):
				destination[1] = destination[1]*attsdpd2_mod
				# modify lateral connections between Attention and D2 deep pyramidal
        ...
        for module in modules.keys():
            ...
            fs_abs_syn.append(open('./output/' +module + '_abs_syn.out', 'w'))
            fs_signed_syn.append(open('./output/' +module + '_signed_syn.out', 'w'))
        ...
        if useTVBConnectome == True:
            ...
            numpy.save("./output/tvb_neuronal.npy", TVB_elec)
            numpy.save("./output/tvb_abs_syn.npy", TVB_abs_syna)
            numpy.save("./output/tvb_signed_syn.npy", TVB_signed_syna)
